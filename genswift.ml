(*
 *  Haxe/Swift Compiler
 *  Copyright (c)2014 Băluță Cristian
...
class sourceWriter write_func close_func =
	...
	method write_header_import (module_path:path) (class_path:path) = 
		...
		this#write ("import " ^ !steps ^ (joinClassPath class_path "/") ^ "\n")
...
let unsupported p = error "This expression cannot be generated to Swift" p
...
(* TODO: Generate resources that Swift can understand *)
...
and generateExpression ctx e =
	...
	(* print_endline ("> Generating expression : "^(Type.s_expr_kind e)); *)
	match e.eexpr with
	| TConst c ->
...
let generateHXObject common_ctx =
	let m_file = newSourceFile (srcDir common_ctx) ([],"HXObject") ".swift" in
	m_file#write ("//
//  HXObject
//
//  Source generated by Haxe Swift target
			...
			if not class_def.cl_extern then begin
				...
				if is_new_module_m then begin
					...
					if not class_def.cl_interface then begin
						...
						files_manager#register_source_file module_path ".swift";
